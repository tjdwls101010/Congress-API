...
CREATE TABLE IF NOT EXISTS bill_stages (
    ...
    --    (2026-08-10 전면 재수집 실측)
    --
    --      단계      행수     위원회        회부일   처리일  결과
    --      ────────────────────────────────────────────────────────
    --      소관위    20,703   100%          95%      32%     32%
    --      소위      16,434   47%           98%      32%     32%
    --      체계자구   1,501   100%(법사위)  100%     97%     97%
    --      본회의     5,885   없음          없음     100%    100%
    --
    --    **본회의의 위원회·회부일은 원천에 칸이 아예 없다** — 0%는 우리가 못 읽은 것이
    --    아니라 그런 사실이 없는 것이다. 반대로 낮은 값(소관위 처리 32%)은 *아직 그
    --    사건이 안 일어난 것*이고, 소관위의 회부 95% ↔ 처리 32% 사이가 계류 14,197건의
    --    실체다. **소위의 위원회 47%만이 진짜 결측이다** — 원천이 소위명을 안 적는다.
    --    체계자구가 100%인 것은 표가 아니라 캡션('법사위 체계자구 심사정보')에서
    --    채우기 때문이다. v1은 이 칸을 안 채워 1,501행 전부 NULL이었다.
);
