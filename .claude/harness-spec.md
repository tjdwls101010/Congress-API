...
### 2026-08-10 — congress 스킬 v2 (같은 세션, 하네스 밖)

같은 날 스킬 내부를 v2로 다시 썼다(스키마 4층화·새 원천 둘·파서 전면 수정·갱신/락/감사·전면 재수집). **하네스 층 자체는 안 바뀌었다** — B1~B7의 배치와 근거가 그대로다. 다만 B5가 실제로 값을 냈는지가 이 스펙의 관심사라 그것만 적는다.

**B5는 계획대로 2단계를 지켰다.** 1단계(스키마 재작성) 동안 꺼 두고 그 판정 직후 켰더니, 파서 3종을 뜯는 2단계 내내 `selftest`가 초록불로 남아 훅이 한 번도 헛울지 않았다. 7단계에 뒀으면 이 구간을 통째로 놓쳤다.

**B7이 실제로 쓰였다.** 재수집 중간에 찾은 결함 다섯 개 중 넷이 `rules/parsers.md`에 이미 적혀 있던 함정의 재발이었다 — 특히 "하위 실패는 성공하면 반드시 지워라"는 **규칙을 써 놓고 코드가 어기고 있었다.** 규칙이 코드를 자동으로 고치지는 않지만, 원장을 세어 보게 만든 것이 그 문장이다.

**원천이 바뀌어 `verify.py`가 처음으로 빨간불을 냈다.** 회의록 본문이 자기 id(pdf/xml 링크)를 안 주기 시작해서 `fetch_body`의 대조 가드가 무력해졌고, 그 상태로 받은 2,047건 중 28건이 다른 회의의 본문이었다. 가드를 막는 쪽으로 바꾸지 않고 `verify.py`의 단언으로 드러냈다 — 원천 변화를 잡는 것이 그 도구의 일이고, 지금 그 단언이 빨간 것이 정확한 상태 표시다.
